...
from models import ConsentRequest, ConsentResponse, LoanApplicationRequest
...
class BusinessFeatures(BaseModel):
    ...
    gst_turnover_lakhs_avg: float = Field(
        gt=0, description="Average monthly GST turnover in lakhs"
    )
    gst_turnover_growth_yoy_pct: float = Field(
        ge=-100, le=500, description="YoY turnover growth %"
    )
    gst_filing_delay_days_avg: float = Field(
        ge=0, le=365, description="Average GST filing delay in days"
    )
    gst_filing_consistency_pct: float = Field(
        ge=0, le=100, description="% of returns filed on time"
    )
    upi_aa_avg_monthly_inflow_lakhs: float = Field(
        gt=0, description="Average monthly UPI/AA inflow in lakhs"
    )
    upi_aa_avg_monthly_outflow_lakhs: float = Field(
        ge=0, description="Average monthly UPI/AA outflow in lakhs"
    )
    cash_flow_volatility_cv: float = Field(
        ge=0, le=5, description="Coefficient of variation of monthly cash flow"
    )
    bounce_rate_pct: float = Field(ge=0, le=100, description="% of payment bounces")
    avg_bank_balance_runway_days: float = Field(
        ge=0, le=3650, description="Days of runway at current burn rate"
    )
    epfo_employee_count: int = Field(
        ge=0, description="Number of EPFO-registered employees"
    )
    epfo_employee_growth_yoy_pct: float = Field(
        ge=-100, le=500, description="YoY employee headcount growth %"
    )
    epfo_pf_compliance_score: float = Field(
        ge=0, le=100, description="PF compliance score (0-100)"
    )
    epfo_avg_wage_growth_yoy_pct: float = Field(
        ge=-100, le=500, description="YoY average wage growth %"
    )
    ...
    def outflow_must_be_plausible(self) -> "BusinessFeatures":
        ...
        """
        ratio = (
            self.upi_aa_avg_monthly_outflow_lakhs / self.upi_aa_avg_monthly_inflow_lakhs
        )
        if ratio > 5.0:
            raise ValueError(
...
def create_consent(req: ConsentRequest):
    ...
    return ConsentResponse(
        ...
        **{k: v for k, v in CONSENTS[consent_id].items() if k != "purpose"},
    )
...
def approve_consent(consent_id: str):
    ...
    return ConsentResponse(
        consent_id=consent_id, **{k: v for k, v in consent.items() if k != "purpose"}
    )
